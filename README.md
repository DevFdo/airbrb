...
In October of 2025, after the financial success of your messaging platform Slackr, you were invited to speak at several tech talks across the country. During these travels, a _series of uncomfortable hotel stays_ inspired your next brilliant startup idea: a property renting service **AirBrB**.

You and your friends decide on the functionality, and write a RESTful specification, outsourcing the backend work to another company to focus on building the frontend yourselves. You've decided to use ReactJS, with the goal of building out a polished, single-page application that meets the requirements outlined in **Section 2**.

Additionally, you've noted that:

- Given your MVP will only be demonstrated once, the frontend must be **thoroughly tested**.

- To meet modern expectations, a high standard of UI, UX, and accessibility must be maintained.

- The app should be deployed on [Vercel](https://vercel.com/) to facilitate the presentation of your product.

This assignment is closely modelled off the popular property renting platfrom [Airbnb](https://www.airbnb.com.au/). If you're not familiar with the service, we would recommend spending the time to get a feel for how it works.
...
Navigate to the `frontend` folder and run `npm install` in that folder to install the dependencies. Then run `npm run dev` to start the ReactJS app.

You will not need to do any meaningful work in the backend. However, some properties that the backend takes in are defined as blank objects. These are objects (referred to as metadata) that can be defined by you, as the backend will simply store your object on some routes and then return it to you on other routes (i.e. the backend doesn't need to understand the schema of some objects you pass it).
...
<table>
    ...
    <td>
      <ul>
        <li>Features implemented that satisfy requirements as outlined in `2.1`, `2.2`, `2.3`, `2.4`, and `2.5`, `2.6`` and tasks marked 🙉🙉🙉 (for pairs) .</li>
  ...
  <tr>
    ...
    <td>
      <ul>
        <li>Two thirds (10%) of the marks received from complying with requirements in section `2.8` in relation to **component testing**</li>
        <li>One third (5%) of the marks received from complying with requirements in section `2.8` in relation to **ui testing**</li>
  ...
  <tr>
    ...
    <td>
      <ul>
        ...
        <li><b>Expectations on solo groups will be half of that of pairs to achieve the same mark.</b></li>
        <li>If you are working individually and complete all pair tasks (marked 🙉🙉🙉) in it's entirety, you can receive full marks for bonus marks.</li>
      </ul>
    </td>
  </tr>
</table>
